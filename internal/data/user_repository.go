...
func (ur *UserRepository) GetAll(ctx context.Context) ([]user.User, error) {
	q := `SELECT id, first_name, last_name, username, email, picture, created_at, updated_at FROM users;`
...
func (ur *UserRepository) Create(ctx context.Context, u *user.User) error {
	...
	if u.Picture == "" {
		u.Picture = "https://placekitten.com/300/300"
	}
...
func (ur *UserRepository) Update(ctx context.Context, id uint, u user.User) (user.User, error) {
	q := `
